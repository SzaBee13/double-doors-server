...
1. Set `serverWideEnabled: false` in `plugins/DoubleDoors/config.yml` and run `/doubledoors reload` to disable behavior server-wide. Alternatively, verify the current state before using `/doubledoors server-toggle`.
