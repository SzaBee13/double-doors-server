...
The SQLite file is created relative to the proxy working directory. Ensure `plugins/DoubleDoors/` is writable by the proxy process.
...
CREATE USER IF NOT EXISTS 'dd_user'@'%' IDENTIFIED BY 'replace-this-password';
GRANT SELECT, INSERT, UPDATE, CREATE ON doubledoors.* TO 'dd_user'@'%';
FLUSH PRIVILEGES;
```
...
| `sql.jdbcUrl` | `jdbc:sqlite:plugins/DoubleDoors/doubledoors.db` | SQLite or MySQL JDBC URL. |
