...
DoubleDoors can optionally send anonymous usage metrics via FastStats (e.g., which features are enabled, server language, detected plugins). All data is public at https://faststats.dev/project/double-doors-server/minecraft-plugin. Disable with `enableAnonymousTracking: false`.
