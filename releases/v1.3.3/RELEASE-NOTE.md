...
2. Replace the old jars with the `1.3.3` release artifacts.
3. Make sure your build environment uses Java 25 if you are compiling from source.
4. Review the new config keys in `config.yml`, especially the WorldGuard, sound, particle, and filter options.
5. Test `/doubledoors debug` and `/doubledoors preview` on a live server to verify the new behavior.
