...
public final class PluginConfig {
   ...
   */
  public void reload() {
    boolean enableRecursiveOpening = plugin
      .getConfig()
      .getBoolean("enableRecursiveOpening", true);
    int recursiveOpeningMaxBlocksDistance = plugin
      .getConfig()
      .getInt("recursiveOpeningMaxBlocksDistance", 10);
    ...
    boolean enableFenceGates = plugin
      .getConfig()
      .getBoolean("enableFenceGates", true);
    boolean enableTrapdoors = plugin
      .getConfig()
      .getBoolean("enableTrapdoors", true);
    boolean enableVillagerLinkedDoors = plugin
      .getConfig()
      .getBoolean("enableVillagerLinkedDoors", true);
    boolean enableAutoClose = plugin
      .getConfig()
      .getBoolean("enableAutoClose", false);
    int autoCloseDelaySeconds = plugin
      .getConfig()
      .getInt("autoCloseDelaySeconds", 5);
    ...
    boolean enableKnockFeature = plugin
      .getConfig()
      .getBoolean("enableKnockFeature", true);
    int knockDistanceBlocks = plugin
      .getConfig()
      .getInt("knockDistanceBlocks", 12);
    ...
    boolean serverWideEnabled = plugin
      .getConfig()
      .getBoolean("serverWideEnabled", true);
    boolean playPartnerSound = plugin
      .getConfig()
      .getBoolean("playPartnerSound", true);
    boolean enablePartnerParticles = plugin
      .getConfig()
      .getBoolean("enablePartnerParticles", false);

    String genericSoundRaw = plugin.getConfig().getString("partnerSound", "");
    Sound genericPartnerSound = parseSound(genericSoundRaw);
    Sound doorPartnerSound = parseSound(
      plugin.getConfig().getString("partnerSoundOverrides.doors", "")
    );
    Sound gatePartnerSound = parseSound(
      plugin.getConfig().getString("partnerSoundOverrides.fenceGates", "")
    );
    Sound trapdoorPartnerSound = parseSound(
      plugin.getConfig().getString("partnerSoundOverrides.trapdoors", "")
    );

    String particleName = plugin
      .getConfig()
      .getString("partnerParticle", "END_ROD");
    Particle partnerParticle = parseParticle(particleName, Particle.END_ROD);
    int partnerParticleCount = plugin
      .getConfig()
      .getInt("partnerParticleCount", 6);
    ...
    String previewParticleName = plugin
      .getConfig()
      .getString("previewParticle", "WAX_OFF");
    Particle previewParticle = parseParticle(
      previewParticleName,
      Particle.WAX_OFF
    );
    int previewParticleCount = plugin
      .getConfig()
      .getInt("previewParticleCount", 18);
    ...
    int previewDurationTicks = plugin
      .getConfig()
      .getInt("previewDurationTicks", 60);
    ...
    int extraAnimationDelayTicks = plugin
      .getConfig()
      .getInt("animationSyncExtraDelayTicks", 0);
    ...
    int lookupCacheTtlMillis = plugin
      .getConfig()
      .getInt("lookupCacheTtlMillis", 1200);
    ...
    int interactionCooldownMillis = plugin
      .getConfig()
      .getInt("interactionCooldownMillis", 250);
    ...
    LocationMode locationMode = parseLocationMode(
      plugin.getConfig().getString("locationFilter.mode", "DISABLED")
    );
    Set<String> configuredLocations = normalizeLocationEntries(
      plugin.getConfig().getStringList("locationFilter.locations")
    );

    RegionMode regionMode = parseRegionMode(
      plugin.getConfig().getString("worldGuardRegionFilter.mode", "DISABLED")
    );
    Set<String> configuredRegions = normalizeRegionEntries(
      plugin.getConfig().getStringList("worldGuardRegionFilter.regions")
    );
    String worldGuardCustomFlag = normalizeLower(
      plugin.getConfig().getString("worldGuardCustomFlag", "double-doors-allow")
    );
    boolean worldGuardRespectBuildPermission = plugin
      .getConfig()
      .getBoolean("worldGuardRespectBuildPermission", true);
    boolean worldGuardRespectUseFlag = plugin
      .getConfig()
      .getBoolean("worldGuardRespectUseFlag", true);

    boolean griefPreventionRequireBuildForLinkedDoors = plugin
      .getConfig()
      .getBoolean("griefprevention.requireBuildForLinkedDoors", false);

    boolean enableAnonymousTracking = plugin
      .getConfig()
      .getBoolean("enableAnonymousTracking", true);

    boolean debug = plugin.getConfig().getBoolean("debug", false);

    boolean sqlEnabled = plugin.getConfig().getBoolean("sql.enabled", false);
    String sqlJdbcUrl = plugin
      .getConfig()
      .getString(
        "sql.jdbcUrl",
    ...
    boolean migrateYamlToSql = plugin
      .getConfig()
      .getBoolean("sql.migrateFromYaml", true);
    long heartbeatSeconds = plugin
      .getConfig()
      .getLong("sql.proxyHeartbeatMaxAgeSeconds", 180L);
    if (heartbeatSeconds < 15L) {
      heartbeatSeconds = 15L;
    }
    long proxyHeartbeatMaxAgeMillis = heartbeatSeconds * 1000L;

    boolean updateCheckerEnabled = plugin
      .getConfig()
      .getBoolean("updateChecker.enabled", true);
    int updateCheckerScheduleSeconds = plugin
      .getConfig()
      .getInt("updateChecker.checkScheduleSeconds", 3600);
    ...
    boolean updateCheckerNotify = plugin
      .getConfig()
      .getBoolean("updateChecker.notify", true);

    String configuredLanguage = plugin
      .getConfig()
    ...
    boolean perPlayerLocaleEnabled = plugin
      .getConfig()
      .getBoolean("perPlayerLocaleEnabled", false);
  ...
  public Sound getPartnerSound(OpenableType type) {
    return switch (type) {
      case DOOR -> snapshot.doorPartnerSound() != null
        ? snapshot.doorPartnerSound()
        : snapshot.genericPartnerSound();
      case FENCE_GATE -> snapshot.gatePartnerSound() != null
        ? snapshot.gatePartnerSound()
        : snapshot.genericPartnerSound();
      case TRAPDOOR -> snapshot.trapdoorPartnerSound() != null
        ? snapshot.trapdoorPartnerSound()
        : snapshot.genericPartnerSound();
